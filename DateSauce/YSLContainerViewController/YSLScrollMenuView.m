...
- (void)setItemTitleArray:(NSArray *)itemTitleArray
{
    if (_itemTitleArray != itemTitleArray) {
        ...
        for (int i = 0; i < itemTitleArray.count; i++) {
            ...
            CGRect imgframe = CGRectMake(40, 0, kYSLScrollMenuViewWidth-80,30);
            UILabel *itemView = [[UILabel alloc] initWithFrame:frame];
            UIImageView *itemImageView = [[UIImageView alloc]initWithFrame:imgframe];
            if(i == 0){
                CGRect imgframe = CGRectMake(10, 0, kYSLScrollMenuViewWidth-80,30);
                if(IS_IPHONE_5){
                    imgframe = CGRectMake(30, 0, kYSLScrollMenuViewWidth-80,30 );
                }
            [itemImageView setFrame:imgframe];
            itemImageView.image = [UIImage imageNamed:@"user-black-close-up-shape (1)"];
            }
            else if(i == 1){
                CGRect imgframe = CGRectMake(40, 0, kYSLScrollMenuViewWidth-80,30);
                [itemImageView setFrame:imgframe];
                itemImageView.image = [UIImage imageNamed:@"tinder_icon-1"];
            }
            else if(i == 2){
                CGRect imgframe = CGRectMake(80, 0, kYSLScrollMenuViewWidth-80,30);
                if(IS_IPHONE_5){
                    imgframe = CGRectMake(50, 0, kYSLScrollMenuViewWidth-80,30 );
                }
                [itemImageView setFrame:imgframe];
                itemImageView.image = [UIImage imageNamed:@"speech-bubbles-comment-option (1)"];
            }
            ...
            itemView.textColor = [UIColor clearColor];
            //itemView.textColor = _itemTitleColor;
            [views addObject:itemView];
            
            [imgviews addObject:itemImageView];
        ...
        self.itemImageViewArray = [NSArray arrayWithArray:imgviews];
            ...
            currentIndex:(NSInteger)currentIndex
{
    ...
    for (int i = 0; i < self.itemViewArray.count; i++) {
        ...
        if (i == currentIndex) {
         ...
         for (int i = 0; i < self.itemImageViewArray.count; i++) {
            
        UIImageView *imageView = self.itemImageViewArray[i];
             
        if(currentIndex == 0 && i == currentIndex)
            imageView.image = [UIImage imageNamed:@"user-black-close-up-shape"];
        else if(currentIndex == 1 && i == currentIndex)
            imageView.image = [UIImage imageNamed:@"tinder_icon"];
        else if(currentIndex == 2 && i == currentIndex)
            imageView.image = [UIImage imageNamed:@"speech-bubbles-comment-option"];
        else{
            if(i == 0)
                imageView.image = [UIImage imageNamed:@"user-black-close-up-shape (1)"];
            else if(i == 1)
                imageView.image = [UIImage imageNamed:@"tinder_icon-1"];
            else if(i == 2)
                imageView.image = [UIImage imageNamed:@"speech-bubbles-comment-option (1)"];
        }
         }
    }
}
